...
Aufruf aus dem `Modellarchitektur/`-Verzeichnis:
...
def _check_data_files(here: Path) -> bool:
    ...
    if ".claude" in parts and "worktrees" in parts:
        try:
            ...
            candidates.append(Path(*parts[:wt_idx]) / "Modellarchitektur" / "data")
        except ValueError:
            pass
    for d in candidates:
...
def _check_cache_files(here: Path) -> bool:
    ...
    if missing:
        ...
        print("     Lösung: python tools/data_fetch/03_fetch_brent_crude.py + python tools/data_fetch/04_fetch_bundesbank_svensson.py")
        return False
...
def main() -> None:
    ...
    if not app_path.exists():
        ...
        print("     Bitte aus dem 'Modellarchitektur/'-Verzeichnis starten.")
        sys.exit(1)
